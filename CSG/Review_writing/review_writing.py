...
from curp.dataset import CSG_review, simple_collate_fn
...
def load_model(config, device):
    ...
    # Move custom modules to the same device as LLaMA input embeddings and convert to FP16
...
def predict(model, llama_tokenizer, data_loader, device, max_new_tokens=512):
    results = []
...
def inference_from_dataset(config, device):
    ...
    val_dataset = CSG_review(
        raw_path=config["val_data_path"],
        llama_tokenizer=llama_tokenizer,
        bert_tokenizer=bert_tokenizer,
        bert_max_len=300,
        llama_max_len=1000,
        llama_max_answer_len=16,
    )
    ...
    results = predict(model, llama_tokenizer, val_dataloader, device, max_new_tokens=200)

    # Save results to file
    output_path = os.path.join(config["output_dir"], "notune_1.json")
...
def main():
    ...
    config = {
        ...
        "val_data_path": "/root/wangliang/Understanding-Generation/curp/CSG/Review_writing/val_sampled_1200.json",
