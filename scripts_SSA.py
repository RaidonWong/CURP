...
from curp.dataset import SSA_data,simple_collate_fn
from curp.model import SSA_model
from curp.utils import setup_seed,train_one_epoch_accelerate, evaluate_accelerate



def main(config):
    # --- 初始化 initialize Accelerator ---
    ...
    print(f"Process {accelerator.process_index} use device: {device}")
    ...
    # --- initial Dataset (不变) ---
    train_dataset = SSA_data(
    data_path=config["train_data_path"],  # training set path
    encoder=bert_tokenizer,
    decoder=llama_tokenizer,
    decoder_max_question_len=180,  # llama max question length
    decoder_max_answer_len=150 # llama max answer length
)

    val_dataset = SSA_data(
    data_path=config["val_data_path"],  # val set path
    encoder=bert_tokenizer,
    decoder=llama_tokenizer,
    decoder_max_question_len=180,  # llama max question length
    decoder_max_answer_len=150 # llama max answer length
    ...
    freeze_bert=True,  # freeze BERT model
    bert_final_seq_len=150  # BERT max sequense length
)
    model = model.half()  # float 16
    model.to(accelerator.device)  # move the model to device
...
# group by name, each group with different learning rate
    ...
    for epoch in range(config["num_epochs"]):
        ...
        train_loss = train_one_epoch_accelerate(accelerator, model, train_dataloader, optimizer)
        if accelerator.is_main_process:
        ...
        if config["val_data_path"] and (epoch + 1) % config["eval_interval"] == 0:
            ...
            val_loss = evaluate_accelerate(accelerator, model, val_dataloader) 
            ...
            # 所有进程现在都从 decision_list[0] 获取到了是否应该保存的指令     all process get the instruction whether to save from decision_list[0]
            should_save_globally = decision_list[0]
            
            # 5. 所有进程根据广播的决策执行操作         all process operate according to the broadcast
            if should_save_globally:
                ...
                try:
                    ...
                    if accelerator.is_main_process:
                        ...
                        save_weights_path = os.path.join(save_dir, 'pytorch_model.bin') # 或 'model_weights.pth'
                        # get state_dict
                        ...
                        for name, param in state_dict.items():
                            ...
                            if 'lora' in name or 'mlp1' in name  or 'codebook' in name or 'stage1' in name or 'stage1' in name:  
                                filtered_state_dict[name] = param
                        # 保存筛选后的权重 save filtered weight
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "train_data_path": "/remote-home/share/lwang_share/data/filtered_dataset_100_128.json",
        "val_data_path": "/remote-home/share/lwang_share/data/filtered_dataset_val_100_128.json",
